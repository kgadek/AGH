...
int main(int argc, char *argv[])
{
    ...
    szukana_litera = 0;	/* Konrad: to aby sie gcc -ansi -pedantic nie czepial... */
	...
	if (!plik) {
		...
		c = getc(plik);
		znaki++;
		if (sprawdzac_litere == 'T' && c == szukana_litera)
		    litera++;
		if (c == '\n')
		    linie++;
    ...
    if (sprawdzac_litere == 'T')
	printf("%6i  %6i  %6i  %6i\n", zbiorcze_linie, zbiorcze_slowa,
	       zbiorcze_znaki, zbiorcza_litera);
    else
	printf("%6i  %6i  %6i\n", zbiorcze_linie, zbiorcze_slowa,
	       zbiorcze_znaki);
    return 0;
