...
void init(struct skipleb *co)
{
    ...
    co->head->nexts =
	(struct skipek **) calloc((size_t)max, sizeof(struct skipek *));
    for (i = max; i > 0; i--)
	co->head->nexts[i - 1] = NULL;
}
...
struct skipleb insert(struct skipleb gdzie, int co)
{
    ...
    int i;
    temp->data = co;
    while ((rand() % 2) && los <= gdzie.wzrost)
	los++;
    if (los > max)
	los = max;
    if (los > gdzie.wzrost)
	gdzie.wzrost = los;
    temp->nexts = (struct skipek **) calloc((size_t)los, sizeof(struct skipek *));
    p = gdzie.head;
    for (i = gdzie.wzrost - 1; i >= 0; i--) {
	while (p->nexts[i] && p->nexts[i]->data <= co)
	    p = p->nexts[i];
	if (i <= los - 1) {
...
struct skipleb del(int co, struct skipleb gdzie)
{
	...
	while (p->nexts[i] && p->nexts[i]->data < co)
	    p = p->nexts[i];
	if (p->nexts[i] && p->nexts[i]->data == co)
	    p->nexts[i] = p->nexts[i]->nexts[i];
    }
    while (p->nexts[0] && p->nexts[0]->data != co)
	p = p->nexts[0];
    if (p->nexts[0]) {
    ...
    if (!gdzie.head->nexts[gdzie.wzrost - 1])
	gdzie.wzrost--;
    return gdzie;
}
