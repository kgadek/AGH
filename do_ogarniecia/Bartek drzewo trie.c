...
node* find( char* txt, node** where)
{ int i;
	
	for (i=0; txt[i]; i++)
	{
		while(*where && txt[i]!=(*where)->val) where=&((*where)->next);
		if (*where && txt[i+1])  where=&((*where)->child);
	}
...
void insert( char* txt, node** p)
{
	char c=*txt;
	while(*p && c!=(*p)->val) p=&((*p)->next);
	if(!*p) /* next jest nulem, nie ma litery */ 
	{
		*p=(node*)malloc(sizeof(node));
		(*p)->val = c; (*p)->next = (*p)->child = NULL;
		if (!*(txt+1)) (*p)->end = 1;
	}
		/*p wskazuje na wskaznik do szukanej litery*/
	if(*(txt+1)) insert(++txt, &((*p)->child));
}

	
void del(char* txt, node* where)
{
