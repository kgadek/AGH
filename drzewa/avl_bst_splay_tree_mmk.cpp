...
namespace mmk {
  ...
  template < typename Data > Tree < Data >::Tree():
    ...
    template < typename Data > bool Tree < Data >::cleanup(Node < Data >
							   *p) {
	if (!p)
	    return false;
	cleanup(p->right);
  ...
  template < typename Data > BST < Data >::BST():tmpe(0) {
    ...
    template < typename Data > void BST < Data >::plant(Data values[],
							const int &size) {
	...
	*localRoot = n;
    ...
    template < typename Data > void AVL < Data >::plant(Data values[],
							const int &size) {
	for (int i = 0; i < size; ++i)
	    insert(values[i]);
    }
    ...
    template < typename Data > void AVL < Data >::insRepair(Node < Data >
							    *n) {
	for (;;) {
	    if (n->bf < -1 || n->bf > 1) {
		balance(n);
    ...
    template < typename Data > void AVL < Data >::delRepair(Node < Data >
							    *n) {
	if (!n || n->bf == -1 || n->bf == 1)
	    return;
	for (;;) {
